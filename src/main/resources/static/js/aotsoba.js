    ...
    $.ajax({
        ...
        success: function (res) {
            if(res.b){
                initDiv();
                var list = res.result;
                directRevenueShare(list[0].date , list[0].value);
                directRevenueShareSM(list[1].date , list[1].value);
                mainBusinessCashFlowRatio(list[2].date , list[2].value);
                mainBusinessCashFlowRatioSM(list[3].date , list[3].value);
            }
        }
    });
}
...
function initDiv() {
    ...
    $('#directRevenueShare').parent('div').siblings('span').text('主营业务净收入占比分析');
    $('#mainBusinessCashFlowRatio').parent('div').siblings('span').text('主营业务现金流占比分析');
